...
namespace Hello_Console_stud
{
    class Program
    {
        ...
        {   
            ...
            Console.WriteLine("Enter bi-bip text:");
            ...
            if (userText == "")
            {
                ...
                Console.WriteLine("Error. You need enter text.");
            }
            else
            {
                char[] userChar = userText.ToCharArray();
                foreach (char currentChar in userChar)
                {
                    int idCharDict = -1;
                    foreach (string currentDict in Dictionary_arr)
                    {
                        idCharDict++;
                        if (currentChar == currentDict[0])
                        {
                            char[] currentBipChar = Dictionary_arr[1, idCharDict].ToCharArray();
                            foreach (char bipChar in currentBipChar)
                            {
                                if (bipChar == '.')
