...
namespace Hello_Console_stud
{
    class Program
    {
        ...
        {   
            ...
            foreach (var c in a)
            {
                for (int index = 0; index < Dictionary_arr.GetLength(0); index++)
                {
                    if (Dictionary_arr[index, 0] == c.ToString())
                    {
                        res = Dictionary_arr[index, 1].Trim();
                        break;
                    }
                }
                foreach (var ch in res.ToCharArray())
                {
                    if (ch == '-')
                    {
