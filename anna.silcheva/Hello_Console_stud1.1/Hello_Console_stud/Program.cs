...
namespace Hello_Console_stud
{
	class Program
	{
		...
		static void My_Binary()
		{
			...
			while (a > 0)
			{
				hldr = Convert.ToInt32(a % 2);
				a_str = a_str + hldr;
				a = a / 2;
			}

			char[] tochar = a_str.ToCharArray();
    ...
    static void Morse_code()
    {
        ...
        //var charArray = Dictionary_arr.SelectMany(x => x.ToCharArray());
        //Use foreach loop for character array in which
        /* for (int i = 0; i < Dictionary_arr.GetLength(0); i++)
          {
              if num[i]
              string ss = num[i].ToString();
              while (Dictionary_arr[0,i]!=ss[i]) {
                  i++;
              }
          }*/
        ...
        foreach (char c in a)
        {
            for (int i = 0; i < Dictionary_arr.GetLength(0); i++)
            {
                if (Dictionary_arr[i, 0] == c.ToString())
                {
                    res = Dictionary_arr[i, 1].Trim();
                    break;
                }
            }
